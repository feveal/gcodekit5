# Test Migration Status - Issue gcodekit5-4
...
Began systematic migration of all inline `#[cfg(test)]` tests from `src/` to `tests/` directory following AGENTS.md guidelines and the successful visualizer migration pattern (gcodekit5-1).
...
use gcodekit5::firmware::module_name::*;
...
3. Change `use super::*` to `use gcodekit5::{module}::*`
...
- **Reference**: Visualizer migration (gcodekit5-1): 28 → 102 tests
