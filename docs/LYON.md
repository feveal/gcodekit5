...
## Executive Summary
We recommend integrating the `lyon` crate ecosystem to replace the custom vector math and SVG path handling currently in `gcodekit5-camtools` and `gcodekit5-designer`. This will provide robust, industry-standard algorithms for path manipulation, flattening, and tessellation, which are prerequisites for implementing the requested "internal path hatching" feature.
...
### Dependencies
Add `lyon` (v1.0+) to `gcodekit5-camtools` and `gcodekit5-designer`.
...
- [ ] Add `lyon` dependency to `gcodekit5-camtools` and `gcodekit5-designer`.
...
- [ ] Update `gcodekit5-designer` to use `lyon` for its internal shape representation where appropriate.
