...
- **Storage Location**: `~/.config/gcodekit5/custom_tools.json` (Linux/macOS) or `%APPDATA%\gcodekit5\custom_tools.json` (Windows)
...
```rust
use gcodekit5::ui::ToolsManagerBackend;
use gcodekit5::data::tools::{Tool, ToolId, ToolType, ToolMaterial};
...
- **Storage**: `~/.config/gcodekit5/custom_tools.json`
