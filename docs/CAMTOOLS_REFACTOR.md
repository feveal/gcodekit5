...
The CAM (Computer-Aided Manufacturing) tools have been extracted from `gcodekit5-parser` into a new dedicated crate: **`gcodekit5-camtools`**. This separation improves modularity, reduces the parser crate size, and makes CAM tools a first-class citizen in the architecture.

---

## What Moved

### New Crate: `gcodekit5-camtools` (~5,400 LOC)
...
```
gcodekit5-parser (contains all CAM + G-Code parsing)
  ├── gcodekit5-core
...
```
gcodekit5-camtools (CAM tools only)
  └── gcodekit5-core
  
gcodekit5-parser (G-Code parsing only)
  └── gcodekit5-core

gcodekit5-ui
  ├── gcodekit5-camtools ✨ (NEW)
  ├── gcodekit5-core
...
**No circular dependencies** - gcodekit5-camtools depends only on gcodekit5-core, not on parser.
...
### Modified Files
- `/Cargo.toml` - Added gcodekit5-camtools to workspace members
- `crates/gcodekit5-ui/Cargo.toml` - Added gcodekit5-camtools dependency
- `crates/gcodekit5-ui/src/ui/mod.rs` - Updated imports (still uses advanced_features_panel)

### Original Locations (Still Present for Backward Compatibility)

The original files remain in:
- `crates/gcodekit5-parser/src/processing/` (for now)
- `crates/gcodekit5-ui/src/ui/advanced_features_panel.rs` (for now)
...
gcodekit5/
├── crates/
│   ├── gcodekit5-core              (unchanged)
│   ├── gcodekit5-parser            (CAM tools removed)
│   ├── gcodekit5-communication     (unchanged)
│   ├── gcodekit5-camtools ✨       (NEW)
│   └── gcodekit5-ui                (updated to use camtools)
├── docs/
...
## Dependencies Added to `gcodekit5-camtools`

| Dependency | Version | Purpose | Used By |
|-----------|---------|---------|---------|
| gcodekit5-core | workspace | Core types and traits | All modules |
...
- Update any remaining code importing from `gcodekit5_parser::processing::`
- Redirect to `gcodekit5_camtools::` instead
- Remove parser imports for CAM tools

### Phase 3: Clean Up (Future)
- Remove original files from `gcodekit5-parser/src/processing/`
...
- Update all references in docs to use gcodekit5-camtools
...
### Public Exports from `gcodekit5-camtools`
...
```rust
use gcodekit5_parser::processing::{JigsawPuzzleMaker, TabbedBoxMaker};
```

**New imports (preferred):**
```rust
use gcodekit5_camtools::{JigsawPuzzleMaker, TabbedBoxMaker};
```
...
3. **Gradual Migration**
   - Update main app to use gcodekit5_camtools imports
...
- ✅ Created new `gcodekit5-camtools` crate (5,400 LOC)
