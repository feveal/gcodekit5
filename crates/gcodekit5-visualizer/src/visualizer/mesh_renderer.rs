...
use crate::visualizer::mesh_rendering::{MeshCollection, RenderableMesh};
use crate::visualizer::mesh_shaders::{
    MESH_FRAGMENT_SHADER, MESH_VERTEX_SHADER, WIREFRAME_FRAGMENT_SHADER, WIREFRAME_VERTEX_SHADER,
};
use glam::{Mat4, Vec3};
use glow::HasContext;
...
impl MeshRenderer {
    ...
    pub fn new(gl: glow::Context) -> Result<Self> {
        // Compile shaders
        let mesh_shader_program =
            Self::create_shader_program(&gl, MESH_VERTEX_SHADER, MESH_FRAGMENT_SHADER)?;

        let wireframe_shader_program =
            Self::create_shader_program(&gl, WIREFRAME_VERTEX_SHADER, WIREFRAME_FRAGMENT_SHADER)?;
    ...
    pub fn upload_mesh(&mut self, mesh_id: usize, mesh: &RenderableMesh) -> Result<()> {
        ...
        unsafe {
            ...
            // Position (location 0)
            self.gl
                .vertex_attrib_pointer_f32(0, 3, glow::FLOAT, false, 40, 0);
            self.gl.enable_vertex_attrib_array(0);

            // Normal (location 1)
            self.gl
                .vertex_attrib_pointer_f32(1, 3, glow::FLOAT, false, 40, 12);
            self.gl.enable_vertex_attrib_array(1);

            // Color (location 2)
            self.gl
                .vertex_attrib_pointer_f32(2, 4, glow::FLOAT, false, 40, 24);
        ...
        unsafe {
            ...
            if !self.wireframe_mode && !mesh.material.wireframe {
                ...
                self.set_uniform_vec3(
                    &shader_program,
                    "camera_position",
                    &self.lighting.camera_position,
                )?;

                // Set material uniforms
                self.set_uniform_vec4(
                    &shader_program,
                    "material_diffuse",
            ...
            if self.wireframe_mode || mesh.material.wireframe {
                self.gl
                    .draw_elements(glow::LINES, resources.index_count, glow::UNSIGNED_INT, 0);
            } else {
    ...
    fn create_shader_program(
        ...
        unsafe {
            ...
            if !gl.get_shader_compile_status(vs) {
                ...
                gl.delete_shader(vs);
                return Err(MeshRenderError::ShaderError(format!(
                    "Vertex shader: {}",
                    info
                )));
            }
    ...
    fn set_uniform_mat3(
        ...
        unsafe {
            ...
            if let Some(loc) = location {
                self.gl
                    .uniform_matrix_3_f32_slice(Some(&loc), false, matrix);
            }
            Ok(())
        }
    }
    ...
    fn set_uniform_vec4(&self, program: &glow::Program, name: &str, vec: &[f32; 4]) -> Result<()> {
        unsafe {
            ...
            if let Some(loc) = location {
                self.gl
                    .uniform_4_f32(Some(&loc), vec[0], vec[1], vec[2], vec[3]);
            }
            Ok(())
        }
    }
