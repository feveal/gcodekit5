...
fn test_rotate_point_180_degrees() {
    ...
    assert!((result.x - (-1.0)).abs() < 1e-10, "Expected x ≈ -1, got {}", result.x);
    assert!((result.y - 0.0).abs() < 1e-10, "Expected y ≈ 0, got {}", result.y);
}
...
fn test_rotate_point_45_degrees() {
    ...
    assert!((result.x - expected).abs() < 1e-10, "Expected x ≈ {}, got {}", expected, result.x);
    assert!((result.y - expected).abs() < 1e-10, "Expected y ≈ {}, got {}", expected, result.y);
}
...
fn test_rotate_point_with_offset_center() {
    ...
    assert!((result.x - 2.0).abs() < 1e-10, "Expected x ≈ 2, got {}", result.x);
    assert!((result.y - 3.0).abs() < 1e-10, "Expected y ≈ 3, got {}", result.y);
}
...
fn test_rotate_point_360_degrees_returns_to_origin() {
    ...
    assert!((result.x - p.x).abs() < 1e-10, "Expected x ≈ {}, got {}", p.x, result.x);
    assert!((result.y - p.y).abs() < 1e-10, "Expected y ≈ {}, got {}", p.y, result.y);
}
