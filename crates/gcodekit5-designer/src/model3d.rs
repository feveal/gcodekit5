...
impl Triangle3D {
    ...
    pub fn intersects_plane_z(&self, z: f32) -> bool {
        let z_min = self.vertices[0]
            .z
            .min(self.vertices[1].z)
            .min(self.vertices[2].z);
        let z_max = self.vertices[0]
            .z
    ...
    pub fn intersect_plane_z(&self, z: f32) -> Vec<Point3<f32>> {
        ...
        intersections.dedup_by(|a, b| (a.x - b.x).abs() < 1e-6 && (a.y - b.y).abs() < 1e-6);
...
impl Mesh3D {
    ...
    pub fn slice_at_z(&self, z: f32) -> Result<Vec<Shape>> {
        ...
        debug!(
            "Found {} intersection segments at Z = {}",
    ...
    for (i, &(start, end)) in segments.iter().enumerate() {
        ...
        while found_connection {
            ...
            for (j, &(seg_start, seg_end)) in segments.iter().enumerate() {
                ...
                if (current_end.x - seg_start.x).abs() < tolerance
                    && (current_end.y - seg_start.y).abs() < tolerance
                {
                    ...
                    break;
                } else if (current_end.x - seg_end.x).abs() < tolerance
                    && (current_end.y - seg_end.y).abs() < tolerance
                {
