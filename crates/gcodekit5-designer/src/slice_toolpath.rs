...
impl SliceToToolpath {
        ...
        for shape in &slice.shapes {
            match shape {
                Shape::Path(path) => {
                    ...
                    let feed_rate = 1000.0; // mm/min
                    let spindle_speed = 12000; // RPM
                    let segments = vec![
                        crate::toolpath::ToolpathSegment {
                            segment_type: crate::toolpath::ToolpathSegmentType::LinearMove,
                            start: Point::new(min_x, min_y),
                            end: Point::new(max_x, min_y),
                            center: None,
                            feed_rate,
                            spindle_speed,
                            z_depth: Some(-self.params.layer_depth as f64),
                            start_z: None,
                        },
                        crate::toolpath::ToolpathSegment {
                            segment_type: crate::toolpath::ToolpathSegmentType::LinearMove,
                    ...
                    toolpath.segments.extend(segments);
                }
                _ => {
        ...
        for shape in &slice.shapes {
            match shape {
                Shape::Path(path) => {
                    ...
                    // Create simple pocket toolpath with multiple passes
                    let stepover = self.params.tool.diameter * self.params.stepover;
                    let width = max_x - min_x;
                    let passes = ((width / stepover).ceil() as i32).max(1);
                    let feed_rate = 1000.0; // mm/min
                    let spindle_speed = 12000; // RPM
                    
                    for i in 0..passes {
                        let x = min_x + (i as f64 * stepover);
                        if x <= max_x {
                            let segment = crate::toolpath::ToolpathSegment {
                                segment_type: crate::toolpath::ToolpathSegmentType::LinearMove,
                            ...
                            toolpath.segments.push(segment);
                        }
                    }
                }
                _ => {
    ...
    fn generate_engrave_toolpath(&self, slice: &SliceLayer) -> Result<Toolpath> {
        debug!("Generating engrave toolpath for {} shapes", slice.shapes.len());
        ...
        for shape in &slice.shapes {
            match shape {
                Shape::Path(path) => {
                    ...
                    // Create simple centerline engraving
                    let center_x = (min_x + max_x) / 2.0;
                    let feed_rate = 500.0; // Slower for engraving
                    let spindle_speed = 15000; // Higher RPM for engraving
                    let segment = crate::toolpath::ToolpathSegment {
                        segment_type: crate::toolpath::ToolpathSegmentType::LinearMove,
                        start: Point::new(center_x, min_y),
                        end: Point::new(center_x, max_y),
                        center: None,
                        feed_rate,
                        spindle_speed,
                        z_depth: Some(-self.params.layer_depth as f64 * 0.1), // Shallow depth
                        start_z: None,
                    };
                    toolpath.segments.push(segment);
                }
    ...
    fn generate_adaptive_toolpath(&self, slice: &SliceLayer) -> Result<Toolpath> {
        debug!("Generating adaptive toolpath for {} shapes", slice.shapes.len());
    ...
    fn generate_summary(&self, layers: &[LayerToolpath], mesh: &Mesh3D) -> Result<JobSummary> {
        ...
        let total_length: f64 = layers.iter()
...
impl SlicedJob {
    ...
    pub fn save_layer_info(&self, path: &std::path::Path) -> Result<()> {
        let layer_info: Vec<_> = self.layers.iter()
