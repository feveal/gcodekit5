...
<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
  <path d="M 10 10 L 90 90"/>
    ...
    assert!(gcode.contains("Lower Z for next pass"), "Should have Z decrement between passes");
    
    // Verify Z is lowered correctly between passes
    assert!(gcode.contains("G0 Z-0.50"), "Should lower Z by 0.50mm for pass 2");
    assert!(gcode.contains("G0 Z-1.00"), "Should lower Z by 1.00mm for pass 3");
    
    // Each pass should have laser commands
    let m3_count = gcode.matches("M3").count();
    assert!(m3_count >= 3, "Should have at least 3 M3 commands (one per pass)");
...
<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
  <path d="M 10 10 L 90 90"/>
    ...
    assert!(!gcode.contains("Pass 1 of"), "Should have no pass comments when multi_pass is false");
    assert!(!gcode.contains("Lower Z for next pass"), "Should not have Z decrement when single pass");
...
<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
  ...
  <path d="M 20 20 L 80 80"/>
    ...
    assert!(m5_count >= m3_count, "Laser should be disabled after each path");
