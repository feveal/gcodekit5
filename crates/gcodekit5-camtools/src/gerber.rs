...
use gerber_types::{Command, DCode, FunctionCode, Operation, CoordinateNumber, Unit, InterpolationMode, QuadrantMode};
...
impl GerberConverter {
    pub fn generate(params: &GerberParameters, gerber_content: &str) -> Result<String> {
        ...
        writeln!(gcode, "G0 X{:.3} Y{:.3} ; Move to work origin", params.offset_x, params.offset_y)?;
        writeln!(gcode, "G10 L20 P1 X0 Y0 Z0 ; Set current position as work zero")?;
        ...
        let is_copper = matches!(params.layer_type, GerberLayerType::TopCopper | GerberLayerType::BottomCopper);
    ...
    fn create_thick_arc(p1: (f64, f64), p2: (f64, f64), center: (f64, f64), width: f64, clockwise: bool) -> Sketch<()> {
        ...
        if clockwise {
            if end_angle >= start_angle { end_angle -= 2.0 * std::f64::consts::PI; }
        } else {
            if end_angle <= start_angle { end_angle += 2.0 * std::f64::consts::PI; }
        }
    ...
    fn parse_gerber_to_sketches(gerber_content: &str) -> Result<Vec<Sketch<()>>> {
        ...
        let mut sanitized = gerber_content.to_string();
        {
            ...
            let mut count = 0;
            sanitized = re_fs.replace_all(&sanitized, |caps: &regex::Captures| {
                count += 1;
                if count > 1 { String::new() } else { caps[0].to_string() }
            }).to_string();

            let re_mo = Regex::new(r"%MO.*?\*%").unwrap();
            let mut count = 0;
            sanitized = re_mo.replace_all(&sanitized, |caps: &regex::Captures| {
                count += 1;
                if count > 1 { String::new() } else { caps[0].to_string() }
            }).to_string();
        }
        ...
        let doc = match parse(reader) {
            Ok(d) => d,
            Err((d, e)) => {
                warn!("Gerber parser returned error, attempting to use partial document. Error: {:?}", e);
                d
            }
        };
        ...
        if let Some(fmt) = &doc.format_specification {
            ...
            warn!("Found Format Specification: {}.{}", fmt.integer, fmt.decimal);
        } else {
        ...
        warn!("Conversion params: Divisor X: {}, Divisor Y: {}, Unit Scale: {}", divisor_x, divisor_y, unit_scale);
        ...
        let convert_coord = |c: &CoordinateNumber, is_x: bool| -> f64 {
            ...
            let val = if let Some(start) = s.find("nano: ") {
                ...
                let val_str = s.trim_start_matches("CoordinateNumber(")
                               .trim_end_matches(')')
                        ...
                        DCode::Operation(op) => {
                            match op {
                                Operation::Interpolate(coord, offset) => {
                                    // D01 - Draw
                                    let x = coord.as_ref().and_then(|c| c.x.as_ref()).map(|v| convert_coord(v, true)).unwrap_or(current_x);
                                    let y = coord.as_ref().and_then(|c| c.y.as_ref()).map(|v| convert_coord(v, false)).unwrap_or(current_y);
                                    
                                    let width = if let Some(ap) = doc.apertures.get(&current_aperture_code) {
                                        use gerber_types::Aperture as GAperture;
                                        match ap {
                                            GAperture::Circle(c) => c.diameter * unit_scale,
                                            GAperture::Rectangle(r) => (r.x * unit_scale).min(r.y * unit_scale), // Approx
                                            GAperture::Obround(o) => (o.x * unit_scale).min(o.y * unit_scale), // Approx
                                    ...
                                    match interpolation {
                                        InterpolationMode::Linear => {
                                            sketches.push(Self::create_thick_segment((current_x, current_y), (x, y), width));
                                        }
                                        InterpolationMode::ClockwiseCircular | InterpolationMode::CounterclockwiseCircular => {
                                            let i_val = offset.as_ref().and_then(|c| c.x.as_ref()).map(|v| convert_coord(v, true)).unwrap_or(0.0);
                                            let j_val = offset.as_ref().and_then(|c| c.y.as_ref()).map(|v| convert_coord(v, false)).unwrap_or(0.0);
                                            
                                            let cx = current_x + i_val;
                                            let cy = current_y + j_val;
                                            
                                            let clockwise = matches!(interpolation, InterpolationMode::ClockwiseCircular);
                                            sketches.push(Self::create_thick_arc((current_x, current_y), (x, y), (cx, cy), width, clockwise));
                                        }
                                    }
                                ...
                                Operation::Move(coord) => {
                                    // D02 - Move
                                    current_x = coord.as_ref().and_then(|c| c.x.as_ref()).map(|v| convert_coord(v, true)).unwrap_or(current_x);
                                    current_y = coord.as_ref().and_then(|c| c.y.as_ref()).map(|v| convert_coord(v, false)).unwrap_or(current_y);
                                }
                                Operation::Flash(coord) => {
                                    ...
                                    if let Some(ap) = doc.apertures.get(&current_aperture_code) {
                                        ...
                                        let (s, tx, ty): (Sketch<()>, f64, f64) = match ap {
                                            GAperture::Circle(c) => (Sketch::circle((c.diameter * unit_scale)/2.0, 32, None), x, y),
                                            GAperture::Rectangle(r) => {
                                                let w = r.x * unit_scale;
                                                let h = r.y * unit_scale;
                                                (Sketch::rectangle(w, h, None), x - w/2.0, y - h/2.0)
                                            },
                                            GAperture::Obround(o) => {
                                            ...
                                            GAperture::Polygon(p) => (Sketch::circle((p.diameter * unit_scale)/2.0, 32, None), x, y),
                                            _ => (Sketch::circle(0.05, 8, None), x, y),
                                        };
                                        let s = s.transform(&Matrix4::new_translation(&Vector3::new(tx, ty, 0.0)));
                                        sketches.push(s);
                                    }
                                    current_x = x;
                                    current_y = y;
    ...
    fn generate_simple_trace_from_sketches(params: &GerberParameters, sketches: Vec<Sketch<()>>, gcode: &mut String) -> Result<()> {
        if sketches.is_empty() { return Ok(()); }
    ...
    fn generate_isolation_from_sketches(params: &GerberParameters, sketches: Vec<Sketch<()>>, gcode: &mut String) -> Result<()> {
        if sketches.is_empty() { return Ok(()); }
        ...
        for (poly, is_hole) in polylines {
            let offset_val = if is_hole { -isolation_offset } else { isolation_offset };
            let offset_res = panic::catch_unwind(panic::AssertUnwindSafe(|| {
                poly.parallel_offset(offset_val)
            }));
    ...
    fn generate_rubout_from_sketches(params: &GerberParameters, sketches: Vec<Sketch<()>>, gcode: &mut String) -> Result<()> {
        if sketches.is_empty() { return Ok(()); }
        ...
        // 3. Board Area
        let mut board = Sketch::rectangle(params.board_width as f64, params.board_height as f64, None);
            ...
            if let Some(dir) = &params.directory {
                ...
                if let Ok(entries) = fs::read_dir(path) {
                    for entry in entries.flatten() {
                        ...
                        if ext == "gko" || ext == "gm1" || name.contains("edge.cuts") || name.contains("outline") {
        ...
        warn!("Isolation sketch bounds: {:?}", isolation_sketch.bounding_box());
        warn!("Buffered traces bounds: {:?}", buffered_traces.bounding_box());
        let rubout_area = board.difference(&buffered_traces);
        warn!("Rubout area bounds: {:?}", rubout_area.bounding_box());
        warn!("Rubout area multipolygon polygon count: {}", rubout_area.to_multipolygon().0.len());
             ...
             for event in path.iter() {
                 match event {
                     lyon::path::Event::Begin { at } => {
                         ...
                         writeln!(gcode, "G1 Z{:.3} F{:.1}", params.cut_depth, params.feed_rate)?;
                     }
    ...
    fn polylines_to_gcode(params: &GerberParameters, paths: Vec<Polyline<f64>>, gcode: &mut String) -> Result<()> {
        for path in paths {
            if path.vertex_count() < 2 { continue; }
            ...
            let emit_segment = |gcode: &mut String, p1: PlineVertex<f64>, p2: PlineVertex<f64>| -> Result<()> {
