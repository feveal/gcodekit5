fn main() {
    ...
    let include_path = std::path::PathBuf::from("crates/gcodekit5-ui");
    println!(
        "cargo:rustc-env=SLINT_INCLUDE_PATH={}",
        include_path.display()
    );

    // Build Slint UI with include paths to crates/gcodekit5-ui
    let config = slint_build::CompilerConfiguration::new().with_include_paths(vec![include_path]);
    slint_build::compile_with_config("crates/gcodekit5-ui/ui.slint", config).unwrap();
