...
fn main() -> anyhow::Result<()> {
    ...
    let detected_firmware = std::sync::Arc::new(std::sync::Mutex::new(
        None::<gcodekit5::firmware::firmware_detector::FirmwareDetectionResult>,
    ));
    ...
    let designer_mgr = Rc::new(RefCell::new(gcodekit5::DesignerState::new()));
    ...
    let materials_backend = Rc::new(RefCell::new(gcodekit5::ui::MaterialsManagerBackend::new()));
    ...
    let tools_backend = Rc::new(RefCell::new(gcodekit5::ui::ToolsManagerBackend::new()));
    ...
    main_window.on_config_retrieve_settings(move || {
        if let Some(window) = window_weak.upgrade() {
            ...
            // Read response
            let result: gcodekit5_core::Result<Vec<u8>> = comm.receive();
            match result {
                Ok(response) => {
    ...
    let visualizer = Rc::new(RefCell::new(gcodekit5::visualizer::Visualizer2D::new()));
    ...
    main_window.on_refresh_visualization(move |width, height, max_intensity| {
        if let Some(window) = window_weak.upgrade() {
            ...
            let layers = gcodekit5::visualizer::render_intensity_overlay(&vis, width as u32, height as u32, max_intensity);
            ...
            let (grid, grid_size) = gcodekit5::visualizer::render_grid_to_path(&vis, width as u32, height as u32);
            window.set_visualization_grid_data(grid.into());
            window.set_visualizer_grid_size(format!("{:.1}mm", grid_size).into());
            
            let origin = gcodekit5::visualizer::render_origin_to_path(&vis, width as u32, height as u32);
    ...
    main_window.on_create_material(move |material_data| {
        if let Some(window) = window_weak.upgrade() {
            ...
            if let Some(category) = gcodekit5::ui::materials_manager_backend::string_to_category(
                material_data.category.as_ref(),
            ) {
                let mut material = gcodekit5::data::materials::Material::new(
                    gcodekit5::data::materials::MaterialId(material_data.id.to_string()),
                ...
                material.chip_type = gcodekit5::ui::materials_manager_backend::string_to_chip_type(
                    material_data.chip_type.as_ref(),
                );
                material.heat_sensitivity =
                    gcodekit5::ui::materials_manager_backend::string_to_heat_sensitivity(
                        material_data.heat_sensitivity.as_ref(),
                    );
                material.abrasiveness =
                    gcodekit5::ui::materials_manager_backend::string_to_abrasiveness(
                        material_data.abrasiveness.as_ref(),
                    );
                material.surface_finish =
                    gcodekit5::ui::materials_manager_backend::string_to_surface_finish(
                        material_data.surface_finish.as_ref(),
                    );
                material.dust_hazard =
                    gcodekit5::ui::materials_manager_backend::string_to_hazard_level(
    ...
    main_window.on_delete_material(move |id| {
        if let Some(window) = window_weak.upgrade() {
            let mut backend = materials_backend_clone.borrow_mut();
            backend.remove_material(&gcodekit5::data::materials::MaterialId(id.to_string()));
    ...
    main_window.on_filter_by_category(move |category| {
        if let Some(window) = window_weak.upgrade() {
            ...
            if let Some(mat_category) =
                gcodekit5::ui::materials_manager_backend::string_to_category(category.as_ref())
            {
    ...
    main_window.on_create_tool(move |tool_data| {
        if let Some(window) = window_weak.upgrade() {
            ...
            if let Some(tool_type) = gcodekit5::ui::tools_manager_backend::string_to_tool_type(
                tool_data.tool_type.as_ref(),
            ) {
                let tool_id =
                    gcodekit5::data::tools::ToolId(format!("custom_{}", tool_data.number));

                let mut tool = gcodekit5::data::tools::Tool::new(
                    tool_id,
                ...
                if let Some(material) =
                    gcodekit5::ui::tools_manager_backend::string_to_tool_material(
    ...
    main_window.on_update_tool(move |tool_data| {
        if let Some(window) = window_weak.upgrade() {
            ...
            if let Some(tool_type) = gcodekit5::ui::tools_manager_backend::string_to_tool_type(
                ...
                let tool_id = gcodekit5::data::tools::ToolId(tool_data.id.to_string());
    ...
    main_window.on_delete_tool(move |id| {
        if let Some(window) = window_weak.upgrade() {
            let mut backend = tools_backend_clone.borrow_mut();
            backend.remove_tool(&gcodekit5::data::tools::ToolId(id.to_string()));
    ...
    main_window.on_filter_by_tool_type(move |tool_type| {
        if let Some(window) = window_weak.upgrade() {
            let backend = tools_backend_clone.borrow();
            if let Some(tt) =
                gcodekit5::ui::tools_manager_backend::string_to_tool_type(tool_type.as_ref())
            {
...
fn render_gcode_visualization_background_channel(
    ...
    // For now, I'll use the crate path if possible, but I'm in main.rs so I use `gcodekit5::visualizer::...`
    // I'll update lib.rs to export it.
    let intensity_layers = gcodekit5::visualizer::render_intensity_overlay(&visualizer, width, height, max_intensity);
